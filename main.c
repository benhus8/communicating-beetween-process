...
char* get_fifo_file_name(char *process_name_to_find, char *fifo_name) {
    ...
    if((config_file = fopen("config.txt", "r")) < 0) perror("Failed to open configuration file!");
...
int main (int argc, char *argv[]) {
    ...
    while(1) {
        ...
        if(forked_process != 0) {
            ...
            FILE *ls_cmd = popen(command_to_execute, "r");
            if (ls_cmd == NULL)
                {
                fprintf(stderr, "Failed to execute popen!");
                exit(EXIT_FAILURE);
            }

            static char result_buff[2048];
            size_t n;
            while ((n = fread(result_buff, 1, sizeof(result_buff) - 1, ls_cmd)) > 0) {   
            result_buff[n] = '\0';
            }
            if (pclose(ls_cmd) < 0) perror("Failed to execute pclose!");

            int result_fifo = open(fifo_to_send, O_WRONLY);
            if(write(result_fifo, result_buff, sizeof result_buff) < 0) {
                perror("Failed to send result of command!");
